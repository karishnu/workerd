...
Rust does not have to live here. A crate that belongs to a component lives beside that component's C++, in the same package: e.g. workerd's entry point (`workerd-cli`, the `cli*.rs` files in `src/workerd/server/`). Such crates list `srcs` (and `cxx_bridge_hdrs`) explicitly, prefix their files the way the component's C++ files are (`cli.rs` / `cli_bridge.rs` next to `cli-main.c++`), and use the component's C++ namespace for their bridge. This directory holds the crates that have no C++ home of their own, and the in-tree cxx fork.
