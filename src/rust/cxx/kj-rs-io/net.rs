...
enum Spec {
    ...
    #[cfg(unix)]
    Unix(UnixName),
    /// `"loopback:name"`: the queue was looked up in the network's registry at parse time.
    Loopback {
        name: Vec<u8>,
...
impl TokioAddress {
    ...
    async fn parse(text: &[u8], port_hint: u16, loopback: &LoopbackRegistry) -> Result<Self> {
        if loopback.is_enabled()
            && let Some(name) = text.strip_prefix(b"loopback:")
        {
            return Ok(Self {
                spec: Spec::Loopback {
                    name: name.to_vec(),
                    queue: loopback.queue(name)?,
                },
            });
        }
    ...
    fn targets(&self) -> Result<Vec<SocketAddress>> {
        match &self.spec {
            ...
            Spec::Loopback { name, .. } => Ok(vec![SocketAddress::loopback(name)]),
        }
    }
    ...
    fn connect(
        ...
        let loopback = match &self.spec {
            Spec::Loopback { queue, .. } => Some(Arc::clone(queue)),
            _ => None,
        };
        async move {
            match loopback {
                Some(queue) => queue.connect(),
                None => connect_to(target).await,
            }
        }
    }
    ...
    fn listen(&self) -> Result<Box<TokioListener>> {
        ...
        let backend = match &self.spec {
            ...
            Spec::Loopback { name, queue } => ListenerBackend::Loopback {
                name: name.clone(),
    ...
    fn to_display_bytes(&self) -> Vec<u8> {
        match &self.spec {
            ...
            Spec::Loopback { name, .. } => [b"loopback:".as_slice(), name].concat(),
        }
    }
}
...
impl ListenerShared {
    ...
    async fn accept(&self) -> Result<PeerStream> {
        ...
        let (inners, next) = match &self.backend {
            ...
            ListenerBackend::Loopback { name, queue } => {
                return Ok(PeerStream {
                    stream: queue.accept().await?,
                    peer: SocketAddress::loopback(name),
                });
            }
        };
        loop {
...
impl TokioListener {
    ...
    fn local_addr(&self) -> Result<SocketAddress> {
        match &self.shared.backend {
            ...
            ListenerBackend::Loopback { name, .. } => Ok(SocketAddress::loopback(name)),
        }
    }
}
...
mod tests {
    ...
    fn socket_count(listener: &TokioListener) -> usize {
        match &listener.shared.backend {
            ...
            ListenerBackend::Loopback { .. } => panic!("expected a socket listener"),
        }
    }
    ...
    fn loopback_addresses_need_the_registry_enabled() {
        ...
        match (&addr.spec, &again.spec) {
            (Spec::Loopback { queue: a, .. }, Spec::Loopback { queue: b, .. }) => {
