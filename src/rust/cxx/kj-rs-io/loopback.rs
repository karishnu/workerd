...
impl LoopbackRegistry {
    ...
    /// The queue for `name`, created on first use and owned by the calling loop from then on.
    pub fn queue(&self, name: &[u8]) -> Result<Arc<LoopbackQueue>> {
        let owner = current_loop_runtime_id()?;
        let mut queues = lock(&self.shared.queues, "parseAddress")?;
        Ok(Arc::clone(
            queues
                .entry(name.to_vec())
                .or_insert_with(|| Arc::new(LoopbackQueue::new(owner))),
        ))
    }
}
...
impl LoopbackQueue {
    fn new(owner: tokio::runtime::Id) -> Self {
        Self {
            owner,
