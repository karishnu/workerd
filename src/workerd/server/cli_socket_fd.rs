...
#[cfg_attr(
    windows,
    expect(dead_code, reason = "Windows does not check the socket")
)]
pub enum Error {
    NotOpen,
    NotSocket,
...
/// On Windows there is no re-exec to keep the socket for, so the server simply takes it.
#[cfg(windows)]
pub struct InheritedSocket(u32);

#[cfg(windows)]
impl InheritedSocket {
    pub const fn take(fd: u32) -> Result<Self, Error> {
        Ok(Self(fd))
    }
...
mod tests {
    ...
    fn regular_file_is_not_a_socket() {
        ...
        assert!(matches!(
            InheritedSocket::take(file.as_raw_fd().cast_unsigned()),
            Err(Error::NotSocket)
        ));
    }
}
