...
//! `Server` and runs it. C++ calls back into [`cli_process::Process`] for `--watch`.

// Two modules allow `unsafe`: cli_bridge.rs, whose `cxx::bridge` macro expands to FFI
// declarations, and cli_socket_fd.rs, where a descriptor number from the command line becomes an
// owned descriptor. Everything else is safe code over std, socket2 and the bridge.
#![deny(unsafe_code)]
...
use crate::cli_bridge::bridge;
use crate::cli_config::Config;
use crate::cli_process::Executable;
use crate::cli_process::Process;
use crate::cli_socket_fd::InheritedSocket;
use crate::cli_watch::Watcher;
...
pub fn main() -> ExitCode {
    ...
    let result = match embedded_config {
        ...
        None => run_command(&program_name, parse(cli_args::Main::command()), executable),
    };
...
fn run_command(
    ...
    match args.command {
        ...
        cli_args::Command::Fuzzilli {
            config,
            serve_or_test,
        } => {
            if !bridge::fuzzilli_supported() {
            ...
            bridge::run_test(
                ...
                config.into_words(),
                &serve_or_test.into(),
                &bridge::TestOptions::new(
                    cli_args::TestArgs::default(),
                    cli_args::TestFilter::default(),
                ),
                process,
            )
        }
        cli_args::Command::Test {
            config,
            serve_or_test,
            mut test,
        } => {
            ...
            bridge::run_test(
                ...
                config.into_words(),
                &serve_or_test.into(),
                &bridge::TestOptions::new(test, filter),
                process,
            )
        }
        cli_args::Command::PyodideLock => {
            println!("{}", bridge::pyodide_lock()?);
            Ok(0)
        }
        cli_args::Command::MakePyodideBaselineSnapshot {
            python_version,
            output_directory,
...
fn run_compile(
    config: cli_args::ConfigFileArgs,
...
fn run_baseline_snapshot(
    ...
    bridge::run_test(
        ...
        &bridge::ServeOrTestOptions {
            ...
            ..cli_args::ServeOrTestArgs::default().into()
        },
        &bridge::TestOptions::new(
            cli_args::TestArgs::default(),
            cli_args::TestFilter::default(),
...
fn parse<T: FromArgMatches>(command: clap::Command) -> T {
    ...
    let result = hide_unsupported(command.version(version).propagate_version(true))
        .try_get_matches()
...
/// Hides what this build does not support from help: `--perfetto-trace` and `fuzzilli`. Using them
/// is rejected after parsing.
fn hide_unsupported(command: clap::Command) -> clap::Command {
    const PERFETTO_TRACE: &str = "perfetto_trace";

    let perfetto = bridge::perfetto_supported();
    let hide_perfetto = |command: clap::Command| {
        if command
            .get_arguments()
            .any(|arg| arg.get_id() == PERFETTO_TRACE)
        {
            command.mut_arg(PERFETTO_TRACE, |arg| arg.hide(!perfetto))
        } else {
            command
        }
    };

    let mut command = hide_perfetto(command);
    let subcommands: Vec<String> = command
        .get_subcommands()
        .map(|subcommand| subcommand.get_name().to_owned())
        .collect();
    for name in subcommands {
        let fuzzilli = name == "fuzzilli" && !bridge::fuzzilli_supported();
        command =
            command.mut_subcommand(name, |subcommand| hide_perfetto(subcommand.hide(fuzzilli)));
    }
    command
}
...
fn release_version() -> &'static str {
    ...
    VERSION.get_or_init(|| String::from_utf8_lossy(bridge::release_version()).into_owned())
}

fn common_options(program_name: String, global: &cli_args::GlobalArgs) -> bridge::CommonOptions {
    bridge::CommonOptions {
        program_name,
...
fn load_config<T: CommandFactory>(
    ...
    let compiled = bridge::compile_config(
        &config_file,
        &import_paths,
...
fn watcher<T: CommandFactory>(
    serve_or_test: &cli_args::ServeOrTestArgs,
    executable: Option<&Executable>,
) -> Option<Watcher> {
...
/// that does not exist.
fn check_serve_or_test<T: CommandFactory>(serve_or_test: &cli_args::ServeOrTestArgs) {
    if serve_or_test.perfetto_trace.is_some() && !bridge::perfetto_supported() {
        exit_with(&T::command().error(
...
fn serve_options<T: CommandFactory>(
    serve: cli_args::ServeArgs,
) -> (bridge::ServeOptions, Vec<InheritedSocket>) {
    if let Some(name) = cli_args::socket_overridden_twice(&serve) {
        usage_error::<T>(format!(
    ...
    for cli_args::SocketFd { name, fd } in serve.socket_fd_overrides {
        let socket = InheritedSocket::take(fd).unwrap_or_else(|error| {
            let message = match error {
                cli_socket_fd::Error::NotListening => {
        ...
        socket_fd_overrides.push(bridge::SocketFd { name, fd });
        inherited.push(socket);
    }
    let options = bridge::ServeOptions {
...
impl cli_args::Main {
    ...
    const fn command_name(&self) -> &'static str {
        match self.command {
            cli_args::Command::Serve { .. } => "serve",
            cli_args::Command::Compile { .. } => "compile",
            cli_args::Command::Fuzzilli { .. } => "fuzzilli",
            cli_args::Command::Test { .. } => "test",
            cli_args::Command::PyodideLock => "pyodide-lock",
            cli_args::Command::MakePyodideBaselineSnapshot { .. } => {
                "make-pyodide-baseline-snapshot"
            }
        }
    }
}
