...
pub struct ServeOrTestArgs {
    ...
    #[arg(long, value_name = "path>=<categories", value_parser = parse_override)]
...
impl From<Override> for bridge::Override {
    fn from(Override { name, value }: Override) -> Self {
        Self { name, value }
    }
}

impl From<ServeOrTestArgs> for bridge::ServeOrTestOptions {
