  ...
  switch (conf->which()) {
    case config::Worker::MODULES: {
      ...
      auto moduleArray = KJ_MAP(i, kj::zeroTo(modules.size())) -> Worker::Script::Module {
        auto module = conf.addRef().project(
            [i](config::Worker::Reader worker) { return worker.getModules()[i]; });
      ...
      Worker::Script::ModulesSource result{
        .mainModule = kj::arc<kj::String>(kj::str(modules[0].getName())),
    ...
    case config::Worker::SERVICE_WORKER_SCRIPT: {
      ...
      for (auto binding: conf->getBindings()) {
        if (binding.isWasmModule()) {
          globals.add(
              Worker::Script::Module{.name = kj::arc<kj::String>(kj::str(binding.getName())),
                .content = Worker::Script::WasmModule{
                  .body = kj::arc<kj::Array<const byte>>(
                      kj::heapArray<const byte>(binding.getWasmModule()))}});
        }
      }

      return Worker::Script::ScriptSource{
        .mainScript = kj::arc<kj::String>(kj::str(conf->getServiceWorkerScript())),
        .mainScriptName = kj::arc<kj::String>(kj::str(name)),
        .globals = kj::arc<kj::Array<Worker::Script::Module>>(globals.finish()),
      };
    }
...
invalid:
  return Worker::Script::ScriptSource{
    .mainScript = kj::arc<kj::String>(kj::str(""_kj)),
    .mainScriptName = kj::arc<kj::String>(kj::str(name)),
  };
}
...
Worker::Script::Module WorkerdApi::readModuleConf(kj::Arc<config::Worker::Module::Reader> conf,
  ...
  return {.name = kj::arc<kj::String>(kj::str(conf->getName())),
    .content = [&]() -> Worker::Script::ModuleContent {
    switch (conf->which()) {
      case config::Worker::Module::TEXT:
        return Worker::Script::TextModule{kj::arc<kj::String>(kj::str(conf->getText()))};
      case config::Worker::Module::DATA:
        return Worker::Script::DataModule{
          kj::arc<kj::Array<const byte>>(kj::heapArray<const byte>(conf->getData()))};
      case config::Worker::Module::WASM:
        return Worker::Script::WasmModule{
          kj::arc<kj::Array<const byte>>(kj::heapArray<const byte>(conf->getWasm()))};
      case config::Worker::Module::JSON:
        return Worker::Script::JsonModule{kj::arc<kj::String>(kj::str(conf->getJson()))};
      case config::Worker::Module::ES_MODULE:
        ...
        if (featureFlags.getTypescriptStripTypes()) {
          ...
          if (output.success) {
            auto body = ::kj::from<Rust>(output.code);
            return Worker::Script::EsModule{
              .body = kj::arc<kj::Array<const char>>(kj::heapArray<const char>(body))};
          }
          ...
          KJ_IF_SOME(reporter, errorReporter) {
            ...
            return Worker::Script::TextModule{kj::arc<kj::String>(kj::str(""_kj))};
          } else {
...
#endif  // defined(WORKERD_USE_TRANSPILER)
        return Worker::Script::EsModule{
          kj::arc<kj::Array<const char>>(kj::heapArray<const char>(conf->getEsModule().asArray()))};
      case config::Worker::Module::COMMON_JS_MODULE: {
        Worker::Script::CommonJsModule result{
          .body = kj::arc<kj::String>(kj::str(conf->getCommonJsModule()))};
        if (conf->hasNamedExports()) {
          auto names =
        KJ_MAP(name, conf->getNamedExports()) { return kj::arc<kj::String>(kj::str(name)); };
          result.namedExports = kj::arc<kj::Array<kj::Arc<kj::String>>>(kj::mv(names));
        }
        return result;
      }
      case config::Worker::Module::PYTHON_MODULE:
        return Worker::Script::PythonModule{kj::arc<kj::String>(kj::str(conf->getPythonModule()))};
        ...
        KJ_IF_SOME(resolved,
          ...
          KJ_SWITCH_ONEOF(resolved) {
            ...
            KJ_CASE_ONEOF(def, kj::Arc<server::config::Worker::Module::Reader>) {
              ...
              KJ_IF_SOME(id, jsg::Url::tryParse(mod.name->asPtr())) {
                KJ_SWITCH_ONEOF(mod.content) {
                  ...
                  KJ_CASE_ONEOF(content, Worker::Script::TextModule) {
                    return kj::Maybe<kj::OneOf<kj::String, kj::Own<jsg::modules::Module>>>(
                        jsg::modules::Module::newSynthetic(kj::mv(id),
                            ...
                            jsg::modules::Module::newOwnedTextModuleHandler(content.body.addRef()),
                  ...
                  KJ_CASE_ONEOF(content, Worker::Script::JsonModule) {
                    return kj::Maybe<kj::OneOf<kj::String, kj::Own<jsg::modules::Module>>>(
                        jsg::modules::Module::newSynthetic(kj::mv(id),
                            jsg::modules::Module::Type::FALLBACK,
                            jsg::modules::Module::newOwnedJsonModuleHandler(content.body.addRef()),
                            nullptr, jsg::modules::Module::Flags::NONE,
