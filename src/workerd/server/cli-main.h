...
// The C++ half of workerd's command line: the functions the Rust entry point (cli.rs) calls through
// the cxx bridge in cli_bridge.rs. Rust parses the command line and produces the encoded config
// (config-compiler.h compiles schema files); these set up V8 and the Server and run it.
//
// Do not include workerd/server/cli_bridge.rs.h here: it includes this header.
