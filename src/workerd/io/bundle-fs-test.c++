...
namespace workerd {
namespace {
kj::Arc<kj::String> ownString(kj::StringPtr value) {
  return kj::arc<kj::String>(kj::str(value));
}

kj::Arc<kj::Array<const char>> ownChars(kj::StringPtr value) {
  return kj::arc<kj::Array<const char>>(kj::heapArray<const char>(value.asArray()));
}

kj::Arc<kj::Array<const kj::byte>> ownBytes(kj::ArrayPtr<const kj::byte> value) {
  return kj::arc<kj::Array<const kj::byte>>(kj::heapArray<const kj::byte>(value));
}
...
KJ_TEST("Transpiled bundle module bodies outlive the WorkerSource (VULN-136997)") {
  ...
  // has been destroyed. The File must retain the Arc containing those bytes.
  //
