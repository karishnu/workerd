...
struct WorkerSource {
  ...
  // module types.
  struct EsModule {
    kj::Arc<kj::Array<const char>> body;
  };
  struct CommonJsModule {
    kj::Arc<kj::String> body;
    kj::Maybe<kj::Arc<kj::Array<kj::Arc<kj::String>>>> namedExports;
  };
  struct TextModule {
    kj::Arc<kj::String> body;
  };
  struct DataModule {
    kj::Arc<kj::Array<const byte>> body;
  };
  struct WasmModule {
      ...
      KJ_SWITCH_ONEOF(content) {
        ...
        KJ_CASE_ONEOF(content, CommonJsModule) {
          result.content = CommonJsModule{.body = content.body.addRef(),
            .namedExports =
                content.namedExports.map([](const kj::Arc<kj::Array<kj::Arc<kj::String>>>& other) {
            return other.addRef();
          })};
        }
