...
kj::Arc<CompatibilityFlags::Reader> ownFeatureFlags(CompatibilityFlags::Reader source) {
  return cloneArcMessage<CompatibilityFlags>(source);
}
...
TestFixture::TestFixture(SetupParams&& params)
    : waitScope(params.waitScope),
      ...
      featureFlags(ownFeatureFlags(params.featureFlags.orDefault(CompatibilityFlags::Reader()))),
